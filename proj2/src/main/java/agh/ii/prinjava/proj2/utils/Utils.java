...
import agh.ii.prinjava.proj2.model.Moviee;
...
public interface Utils {
    ...
    static List<Moviee> oneToManyByDirector(Moviee m) {
        List<Moviee> movies = new ArrayList<>(m.directors().size());

        for (var d : m.directors()) {
            Moviee mv = new Moviee(m.rank(), m.rating(), m.title(), m.year(), m.rated(), m.runtime(),
    ...
    static List<Moviee> oneToManyByActor(Moviee m) {
        List<Moviee> movies = new ArrayList<>(m.directors().size());
    ...
    static List<Moviee> oneToManyByActorDuo(Moviee m) {
        List<Moviee> movies = new ArrayList<>(m.directors().size());
